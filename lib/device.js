...
module.exports = function(client) {
  ...
  Device.prototype.reportCards = function(_options) {
    var self = this;
    _options = Hoek.applyToDefaults({ limit: 20 }, _options || {});
    Joi.assert(_options, Utils.streamPaginationOptions);
    ...
    return client.get(
      'behavioral',
      'devices/' + this.id + '/report_cards',
      _options).then(function(resp) {
        resp.reportCards = resp.reportCards.map(function(v) {
          return new client.ReportCard(v);
        });
  ...
  Device.prototype.subscriptions = function(_options) {
    var self = this;
    _options = Hoek.applyToDefaults({ limit: 20 }, _options || {});
    Joi.assert(_options, Utils.streamPaginationOptions);
